...
sandbox.o: src/sandbox.cpp src/sandbox.h
	$(CXX) $(CXXFLAGS) -c src/sandbox.cpp

fasttext: $(OBJS) src/fasttext.cc src/main.cc
	$(CXX) $(CXXFLAGS) $(OBJS) src/main.cc -o fasttext

clean:
	rm -rf *.o fasttext.exe test.exe

test: sandbox.o src/test.cpp
	$(CXX) $(CXXFLAGS) sandbox.o src/test.cpp -o test
